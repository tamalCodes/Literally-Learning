...
<i className="text-yellow-500 font-poppins text-[15px] leading-1">
  Transpiling: The process of converting source code from one language to
  another. In the context of Babel, it refers to converting modern JavaScript
  code to an older version that is compatible with a wide range of browsers and
  environments.
</i>
